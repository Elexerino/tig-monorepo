...
pub fn solve_challenge(challenge: &Challenge) -> Result<Option<Solution>> {
    ...
    let initial_solution = nearest_neighbor_heuristic_with_candidates(num_nodes, vehicle_capacity, demands, distance_matrix);
    let initial_solution_rating: i32 = rate_solution(&initial_solution, distance_matrix);

    let mut best_solution: Option<SolutionWithRating> = Some((initial_solution.clone(), initial_solution_rating));

    // Populate pheromone matrix with initial pheromones generated by initial solution
    let initial_total_distance: i32 = initial_solution.iter()
    ...
    for i in 0..ITERATIONS {
        ...
        for ant_index in 0..num_ants {
            let ant_seed = challenge.seed + ant_index as u32;
        ...
        if let Some((_, best_cost)) = best_solution {
            ...
            if iterations_without_improvement >= NO_IMPROVEMENT_THRESHOLD {
                debug_println!("Terminating due to no improvement for {} iterations.", NO_IMPROVEMENT_THRESHOLD);
                break;
            }
        }
    }
...
fn update_pheromone(
    ...
    let l_avg: f64 = solutions.iter().map(|&(_, cost)| cost).sum::<i32>() as f64 / solutions.len() as f64;
...
fn rate_solution(solution: &Vec<Vec<usize>>, distance_matrix: &Vec<Vec<i32>>) -> i32 {
    solution.iter().map(|route| {
        route.windows(2).map(|w| distance_matrix[w[0].min(w[1])][w[0].max(w[1])])
            .sum::<i32>() + distance_matrix[*route.last().unwrap_or(&0).min(&0)][*route.last().unwrap_or(&0).max(&0)]
    }).sum()
}

fn two_opt(route: &Vec<usize>, distance_matrix: &Vec<Vec<i32>>, demands: &Vec<i32>, vehicle_capacity: i32) -> Vec<usize> {
            ...
            for j in i + 1..solution.len() {
                for k in 1..solution[i].len() - 1 {
                    for l in 1..solution[j].len() - 1 {
                        ...
                        let new_demand_i = compute_route_demand_with_swap(&solution[i], demands, k, customer_j);
                        let new_demand_j = compute_route_demand_with_swap(&solution[j], demands, l, customer_i);
...
fn nearest_neighbor_heuristic_with_candidates(
    ...
    for i in 0..num_nodes {
        ...
        candidate_list[i] = distances.into_iter().take(num_candidates).map(|(j, _)| j).collect();
    }
